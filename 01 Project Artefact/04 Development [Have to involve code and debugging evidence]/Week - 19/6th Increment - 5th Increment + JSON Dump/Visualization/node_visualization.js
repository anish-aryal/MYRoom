(function () {
	...
	}, b.version = "1.0.2", "undefined" != typeof this.sigma) throw "An object called sigma is already in the global scope.";
...
function () {
	...
	i.addMethod = function (a, b) {
		...
		return delete this.inNeighborsIndex[d.target][d.source][d.id], Object.keys(this.inNeighborsIndex[d.target][d.source])
			.length || delete this.inNeighborsIndex[d.target][d.source], delete this.outNeighborsIndex[d.source][d.target][d.id], Object.keys(this.outNeighborsIndex[d.source][d.target])
			.length || delete this.outNeighborsIndex[d.source][d.target], delete this.allNeighborsIndex[d.source][d.target][d.id], Object.keys(this.allNeighborsIndex[d.source][d.target])
			.length || delete this.allNeighborsIndex[d.source][d.target], delete this.allNeighborsIndex[d.target][d.source][d.id], Object.keys(this.allNeighborsIndex[d.target][d.source])
			.length || delete this.allNeighborsIndex[d.target][d.source], this.inNeighborsCount[d.target]--, this.outNeighborsCount[d.source]--, this.allNeighborsCount[d.source]--, this.allNeighborsCount[d.target]--, this
...
function (a) {
	...
	sigma.utils.pkg("sigma.classes"), sigma.classes.camera = function (a, b, c, d) {
		...
		var c = this.cameraPosition(a, 0, !0),
			...
			f = this.cameraPosition(a / 4, 0, !0)
				.x,
			g = this.cameraPosition(0, b / 4, !0)
				.y;
		return {
			x1: this.x - e.x - f,
			y1: this.y - e.y - g,
...
function (a) {
	...
	sigma.utils.pkg("sigma.renderers"), sigma.renderers.canvas = function (a, b, c, d) {
		...
		if (r) {
			for (c = p.edges(), d = 0, f = c.length; f > d; d++) g = c[d], !o[g.source] && !o[g.target] || g.hidden || q(g.source)
				.hidden || q(g.target)
				.hidden || this.edgesOnScreen.push(g);
...
function (a) {
	...
	sigma.utils.pkg("sigma.renderers"), sigma.renderers.webgl = function (a, b, c, d) {
		...
		for (d in this.edgeFloatArrays)
			for (e = sigma.webgl.edges[d], a = this.edgeFloatArrays[d].edges, b = 0, c = a.length; c > b; b++) this.edgeFloatArrays[d].array || (this.edgeFloatArrays[d].array = new Float32Array(a.length * e.POINTS * e.ATTRIBUTES)), a[b].hidden || f.nodes(a[b].source)
				.hidden || f.nodes(a[b].target)
				.hidden || e.addEdge(a[b], f.nodes(a[b].source), f.nodes(a[b].target), this.edgeFloatArrays[d].array, b * e.POINTS * e.ATTRIBUTES, g.prefix, this.settings);
...
function () {
	...
	sigma.utils.pkg("sigma.canvas.hovers"), sigma.canvas.hovers.def = function (a, b, c) {
		...
		b.font = (i ? i + " " : "") + l + "px " + (c("hoverFont") || c("font")), b.beginPath(), b.fillStyle = "node" === c("labelHoverBGColor") ? a.color || c("defaultNodeColor") : c("defaultHoverLabelBGColor"), c("labelHoverShadow") && (b.shadowOffsetX = 0, b.shadowOffsetY = 0, b.shadowBlur = 8, b.shadowColor = c("labelHoverShadowColor")), "string" == typeof a.label && (d = Math.round(a[j + "x"] - l / 2 - 2), e = Math.round(a[j + "y"] - l / 2 - 2), f = Math.round(b.measureText(a.label)
			.width + l / 2 + k + 7), g = Math.round(l + 4), h = Math.round(l / 2 + 2), b.moveTo(d, e + h), b.arcTo(d, e, d + h, e, h), b.lineTo(d + f, e), b.lineTo(d + f, e + g), b.lineTo(d + h, e + g), b.arcTo(d, e + g, d, e + g - h, h), b.lineTo(d, e + h), b.closePath(), b.fill(), b.shadowOffsetX = 0, b.shadowOffsetY = 0, b.shadowBlur = 0), c("borderSize") > 0 && (b.beginPath(), b.fillStyle = "node" === c("nodeBorderColor") ? a.color || c("defaultNodeColor") : c("defaultNodeBorderColor"), b.arc(a[j + "x"], a[j + "y"], k + c("borderSize"), 0, 2 * Math.PI, !0), b.closePath(), b.fill());
...
function () {
	...
	sigma.utils.pkg("sigma.canvas.edges"), sigma.canvas.edges.curve = function (a, b, c, d, e) {
		...
		d.strokeStyle = f, d.lineWidth = a[g + "size"] || 1, d.beginPath(), d.moveTo(b[g + "x"], b[g + "y"]), d.quadraticCurveTo((b[g + "x"] + c[g + "x"]) / 2 + (c[g + "y"] - b[g + "y"]) / 4, (b[g + "y"] + c[g + "y"]) / 2 + (b[g + "x"] - c[g + "x"]) / 4, c[g + "x"], c[g + "y"]), d.stroke()
	}
}(),
function () {
	"use strict";
...
function () {
	...
	a.curvedArrow = function (a, b, c, d, e) {
		var f = a.color,
			...
			m = b[g + "x"],
			n = b[g + "y"],
			o = (b[g + "x"] + c[g + "x"]) / 2 + (c[g + "y"] - b[g + "y"]) / 4,
			p = (b[g + "y"] + c[g + "y"]) / 2 + (b[g + "x"] - c[g + "x"]) / 4,
			q = c[g + "x"],
			r = c[g + "y"],
			s = 2.5 * k,
			t = Math.sqrt(Math.pow(q - o, 2) + Math.pow(r - p, 2)),
			u = o + (q - o) * (t - s - l) / t,
			v = p + (r - p) * (t - s - l) / t,
			w = (q - o) * s / t,
			x = (r - p) * s / t;
		...
		d.strokeStyle = f, d.lineWidth = k, d.beginPath(), d.moveTo(m, n), d.quadraticCurveTo(o, p, u, v), d.stroke(), d.fillStyle = f, d.beginPath(), d.moveTo(u + w, v + x), d.lineTo(u + .6 * x, v - .6 * w), d.lineTo(u - .6 * x, v + .6 * w), d.lineTo(u + w, v + x), d.closePath(), d.fill()
	}
}(),
function () {
	"use strict";
...
function (a) {
	...
	sigma.misc.animation.camera = function (c, d, e) {
		...
		return b && (cancelAnimationFrame(a), delete sigma.misc.animation.running[b.frameId], "camera" === b.type && (b.target.isAnimated = !1), "function" == typeof (b.options || {})
			.onComplete && b.options.onComplete()), this
	}, sigma.misc.animation.killAll = function (a) {
		var b, c, d = 0,
			e = "string" == typeof a ? a : null,
			f = "object" == typeof a ? a : null,
			g = sigma.misc.animation.running;
		for (c in g) e && g[c].type !== e || f && g[c].target !== f || (b = sigma.misc.animation.running[c], cancelAnimationFrame(b.frameId), delete sigma.misc.animation.running[c], "camera" === b.type && (b.target.isAnimated = !1), d++, "function" == typeof (b.options || {})
			.onComplete && b.options.onComplete());
		return d
